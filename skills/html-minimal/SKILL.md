...
문서 전반에 반복 등장하는 **주요 논지·가설·트랙 구분자**(예: α/β/γ, 가설 A/B/C, Track 1/2/3)에 한해 색 라벨 사용 허용. 본문 텍스트·배경·구분선에는 금지.

| 변수 | 값 | 제안 용도 |
|---|---|---|
| `--alpha` | `#a3742a` | 1순위 논지 (머스터드 브라운) |
| `--beta` | `#4f7a58` | 2순위 논지 (세이지 그린) |
| `--gamma` | `#7a5a87` | 3순위 논지 (덤덤 퍼플) |

적용 범위는 라벨 배지 / 카드 상단 얇은 stripe / 인라인 태그로 한정. 사용 개수는 문서당 각 10회 이내.
...
| 본문 한글 | IBM Plex Sans KR | 한글 본문 |
| 본문 영문 | Times New Roman | 영문 본문, 인라인 영어 단어·용어 |
| 숫자/코드 전용 | JetBrains Mono | 통계, 금액, %, 코드, DOI, 수식 |

영문 본문은 Times New Roman 우선 적용 (css `font-family: 'Times New Roman', 'IBM Plex Sans KR', serif`). 한글은 자동으로 IBM Plex Sans KR로 fallback. Inter, Arial, Roboto, system-ui 금지.
...
| 요소 | 크기 | 웨이트 | 행간 | 비고 |
|---|---|---|---|---|
| h1 | 3.0rem | 400 (DM Serif) | 1.15 | letter-spacing: -1px |
| h2 | 0.85rem | 600 | 1.4 | 대문자 변환, letter-spacing: 3px |
| h3 | 0.8rem | 600 | 1.4 | 대문자 변환, letter-spacing: 2px |
| 본문 p | 17px | 300 | 1.75 | body 기본 |
| strong | 본문 동일 | 600 | 본문 동일 | |
| 보조 텍스트 | 0.9rem | 300 | 1.6 | 캡션, 설명 |
| 라벨 | 0.78rem | 600 | 1.4 | 대문자, letter-spacing: 1~3px |
| 숫자 대형 | 1.8rem | 400 (JetBrains) | 1.2 | 핵심 수치 |
| 숫자 인라인 | 0.9rem | 400 (JetBrains) | 본문 | 표, 리스트 내 수치 |

700, 800, 900 웨이트 금지. 300, 400, 600만 사용. 좌우 여백이 남을 때는 이 크기 유지, 그래도 스카스 보이면 max-width 980px로 올림 (상한).
...
body {
  ...
  color: var(--ink);
  font-family: 'Times New Roman', 'IBM Plex Sans KR', serif;
  font-weight: 300;
  font-size: 17px;
  line-height: 1.75;
  padding: 3rem;
